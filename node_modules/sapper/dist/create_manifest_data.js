...
var sourcemapCodec = _interopDefault(require('sourcemap-codec'));
...
const rangeError = (...args) => {
    return new RangeError('Invalid range arguments: ' + util__default.inspect(...args));
};
...
micromatch.contains = (str, pattern, options) => {
    if (typeof str !== 'string') {
        throw new TypeError(`Expected a string: "${util__default.inspect(str)}"`);
    }
...
const {encode, decode} = sourcemapCodec;
...
var rollupPluginCssChunks = function svelte(options = {}) {
	const filter = createFilter$1(/\.css$/i, []);
			...
			for (const chunk of Object.values(bundle).reverse()) {
				...
				for (const f of Object.keys(chunk.modules).filter(filter)) {
					if (data.map[f]) {
						...
						sources.push(path__default.relative(options.dir, data.map[f].sources[0]));
...
class RollupCompiler {
    ...
    get_config(mod) {
        return _commonjsHelpers.__awaiter(this, void 0, void 0, function* () {
            ...
            (mod.plugins || (mod.plugins = [])).push(rollupPluginCssChunks({ injectImports: true }));
